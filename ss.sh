...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/ToyoDAdoubiBackup/doubi/master/service/ssrmu -O /etc/init.d/ssrmu; then
			echo -e "${Error} ShadowsocksR kontrol ediş skriptini gurnap bilmedik !" && exit 1
		fi
		chmod +x /etc/init.d/ssrmu
...
Update_Shell(){
	...
	cd "${file}"
	wget -N --no-check-certificate "https://raw.githubusercontent.com/ToyoDAdoubiBackup/doubi/master/ssr.sh" && chmod +x ssrmu.sh
